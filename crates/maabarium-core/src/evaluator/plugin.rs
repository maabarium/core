...
impl ProcessPluginEvaluator {
    pub fn from_blueprint(blueprint: &BlueprintFile) -> Result<Self, EvalError> {
        ...
        let content = std::fs::read_to_string(&manifest_path)
            .map_err(|error| EvalError::Parse(format!("Failed to read plugin manifest {}: {error}", manifest_path.display())))?;
        let manifest: ProcessPluginManifest = toml::from_str(&content)
            .map_err(|error| EvalError::Parse(format!("Invalid plugin manifest {}: {error}", manifest_path.display())))?;

        if manifest.plugin.id.trim().is_empty() {
            return Err(EvalError::Parse("Plugin manifest id cannot be empty".to_owned()));
        }
        if manifest.process.command.trim().is_empty() {
            return Err(EvalError::Parse("Plugin process command cannot be empty".to_owned()));
        }
...
impl Evaluator for ProcessPluginEvaluator {
    async fn evaluate(
        ...
        let request_json = serde_json::to_vec(&request)
            .map_err(|error| EvalError::Parse(format!("Failed to serialize plugin request: {error}")))?;
        ...
        let working_dir = self
            ...
            .working_dir
            .as_deref()
            .map(|path| resolve_relative_path(self.manifest_path.parent().unwrap_or(Path::new(".")), path));
        ...
        let mut child = command
            .spawn()
            .map_err(|error| EvalError::Parse(format!("Failed to start evaluator plugin '{}': {error}", self.manifest.plugin.id)))?;

        if let Some(stdin) = child.stdin.as_mut() {
            stdin
                .write_all(&request_json)
                .await
                .map_err(|error| EvalError::Parse(format!("Failed to write plugin stdin: {error}")))?;
        }
        ...
        let output = tokio::time::timeout(timeout, child.wait_with_output())
            .await
            .map_err(|_| EvalError::Parse(format!("Evaluator plugin '{}' timed out after {}s", self.manifest.plugin.id, self.manifest.plugin.timeout_seconds)))?
            .map_err(|error| EvalError::Parse(format!("Failed while waiting for evaluator plugin '{}': {error}", self.manifest.plugin.id)))?;
        ...
        let response: PluginEvaluationResponse = serde_json::from_slice(&output.stdout)
            .map_err(|error| EvalError::Parse(format!("Invalid evaluator plugin response from '{}': {error}", self.manifest.plugin.id)))?;
        ...
        Ok(ExperimentResult {
            ...
            duration_ms: response.duration_ms.unwrap_or_else(|| started.elapsed().as_millis() as u64),
...
fn resolve_manifest_path(blueprint: &BlueprintFile) -> Result<PathBuf, EvalError> {
    let path = blueprint
        ...
        .ok_or_else(|| EvalError::Parse("Process evaluator blueprints must include evaluator.manifest_path".to_owned()))?;
    Ok(resolve_relative_path(Path::new(&blueprint.domain.repo_path), path))
}
